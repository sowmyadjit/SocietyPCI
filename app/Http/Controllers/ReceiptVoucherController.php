	...
	class ReceiptVoucherController extends Controller
	{
		...
		public function save_rv_no($data)
		{
			...
			if(strcasecmp($data["rv_payment_mode"], "CASH") == 0 || strcasecmp($data["rv_payment_mode"], "INHAND") == 0) {
				...
				if($data["rv_transaction_type"] == "DEBIT") {
					$temp_receipt_voucher_type_field = ReceiptVoucherModel::ADJUSTMENT;
					$rv_no = $this->rv_no->get_next_adjustment_no(["bid"=>$BID]);//["date"=>$id['dte']  - date is optional
					...
					return "no adj no. for ADJ CREDIT";
				}
			}
