<?php
	...
	class DLRepaymentModel extends Model
	{
		...
		{
			return DB::table('fdallocation')->select('Fdid','Fd_CertificateNum','Fd_TotalAmt')
			->where('Fdid','=',$fdid)
			->first();
		}
...
/***************** pigmi paid amount entry *************************/
		...
		{
			return DB::table('personalloan_allocation')->select('PersLoanAllocID','RemainingLoan_Amt','caldate','FirstName','MiddleName','LastName','EMI_Amount','LoanAmt','partpayment_amount','LoanType_Interest','loan_due_interest','EMI_Amount','RemainingInterest_Amt')
		...
		public function GetjlDetail($id)
		{
			...
			$jid=$id['jlAlcID'];
			return DB::table('jewelloan_allocation')->select('JewelLoan_LoanRemainingAmount','JewelLoan_StartDate','JewelLoan_EndDate','FirstName','MiddleName','LastName','JewelLoan_lastpaiddate','partpayment_amount','JewelLoan_LoanAmount','LoanType_Interest','auction_status','loan_due_interest')
		...
		public function PersonalLoanRepay($id)
		{
			...
			$plTran=DB::table('personalloan_repay')->InsertGetId(['PLRepay_PLAllocID'=>$DepAlID,'PLRepay_PaidAmt'=>$id['plpayamt'],'PLRepay_PayMode'=>$id['plPayMode'],'PLRepay_Bid'=>$branch,'PLRepay_Created_By'=>$UID,'PLRepay_Date'=>$RepayDte,'PLRepay_CalculatedInterest'=>$loaninterest,'RemainingInterest_Amt'=>$remaininginterst,'PLRepay_PaidInterest'=>$paidinterest,'PLRepay_Amtpaidtoprincpalamt'=>$payAmt,'PLRepay_EMIremaining'=>$remainigemi,'PL_ReceiptNum'=>$r,'PL_ChequeNO'=>$chequeno,'PL_ChequeDate'=>$chequedate,'PL_BankName'=>$bankname,'PL_BankBranch'=>$bankbranch,'PL_IFSC'=>$ifsc,'PL_CreditBank'=>$bankid]);
...
//				->select("Fdid","Fd_CertificateNum","Fd_TotalAmt")
		...
		public function jl_first_repay($data)
		{
			$jlaccid = $data['jlaccid'];
			$jl_repay = DB::table('jewelloan_allocation')
				->select()
				->where('JewelLoanId','=',$jlaccid)
				->get();
			$n = count($jl_repay);
