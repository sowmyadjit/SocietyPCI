		...
		<div class="box col-md-12">
			<div class="box-inner">
						...
						<div class="row table-row alert alert-info">
							...
							<label class="control-label inline col-md-2">BRANCH:
								<select class="form-control BranchListDD"  id="BranchListDD" name="BranchListDD" onchange="BranchDDChange();">  
									...
									<?php foreach($ExpBranchWise['BranchListexp'] as $key){
							...
							<label class="control-label inline col-md-3">EXPENSE TYPE:
								
								<select class="form-control ExpType"  id="Exptt" name="Exptt">
								<option>EXPENSE</option>
								<option>EXPENSE TRANSFER</option>
								</select>
				...
				<div class='SearchRes'>
					<div  id="toprint">
						...
						<table class="table table-striped table-bordered bootstrap-datatable datatable responsive">
							...
							<tbody>
								...
								@foreach($ExpBranchWise['ExpReportBWData'] as $Exp)
								<tr>
									<td class="hidden">{{ $Exp->id }}</td>
									<td>{{ $Exp->BName }}</td>
									<td><?php $expdate=date("d-m-Y",strtotime($Exp->e_date)); echo $expdate; ?></td>
									<td>{{ $Exp->pay_mode }}</td>
									<td>{{ $Exp->bank }}</td>
									<td>{{ $Exp->Particulars }}</td>
									<td>{{ $Exp->amount }}</td>	
									<td>{{ $Exp->FirstName }}.{{ $Exp->MiddleName }}.{{ $Exp->LastName }}</td>
					...
					<div id='pagei'>
						{!! $ExpBranchWise['ExpReportBWData']->render() !!}
					</div>
				</div>
			</div>
		</div>
