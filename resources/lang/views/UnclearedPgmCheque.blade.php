...
<label class="control-label col-sm-4">Cheque Reject Amount:</label>
  ...
  <tr>
       @foreach ($pgmcheque as $pigmi_transaction)
        <tr>
             ...
             <td>{{ $pigmi_transaction->FirstName }}</td>
             <td>{{ $pigmi_transaction->MiddleName }}</td>
             <td>{{ $pigmi_transaction->LastName }}</td>
			 ...
			 <td>
									<div class="form-group">
										<div class="col-sm-12">
											<input type="button" value="Accept" class="btn btn-info btn-sm accpbtn" href="pgmclearcheque/{{ $pigmi_transaction->PigmiTrans_ID }}"/>
