				...
				<div class="box-content">
					<div  id="toprint">
						...
						<table class="table table-striped bootstrap-datatable datatable responsive">
							...
							@foreach ($PigPayRece['PigPayAmt'] as $PPR)
							...
							<tr>
								<th>Pigmy Total Amount:</th>
								<td>{{ $PPR->PgmTotal_Amt }}</td>
							</tr>
							
							<tr>
								<th>Deducted Commission:</th>
								<td>{{ $PPR->Deduct_Commission }}</td>
							</tr>
							
							<tr>
								<th>Deducted Amount:</th>
								<td>{{ $PPR->Deduct_Amount }}</td>
							</tr>
							
							<tr>
								<th>Total Deducted Amount:</th>
								<td><div class="TotDeduct"></div></td>
							</tr>
...
<script>
	...
	TotAmt={{ $PPR->PgmTotal_Amt }};
	PayableAmt={{ $PPR->PayAmount_PayableAmount }};
	
	
	
	TotDeducted=TotAmt-PayableAmt;
	
	$('.TotDeduct').html(TotDeducted);
	
	//DedCom1={{ $PPR->Deduct_Commission }};
	//DedCom2=number_format(DedCom,2,'.',',');
	//DedCom=floatval(DedCom2);
	//alert(DedCom1,DedCom);
