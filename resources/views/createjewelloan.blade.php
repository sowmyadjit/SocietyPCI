...
<script>
	...
	$('#JewelPayMode').change( function(e) {
		...
		if(mode=="CASH")
		{
			...
			{
				AccNum=$('.SBAccNumTypeAhead').attr('data-value');
				$.ajax({
	...
	$('.JewelSbmBtn').click( function(e) 
	{
		if(indexid==0)
		{
			...
			$.ajax({
				...
				type: 'post',
				data: $('#FormJewelLoanAlloc').serialize()+'&LoanType='+loantyp+'&BranchId='+branch+'&UserId='+user+'&BankId='+bankid+'&accid='+accid,
				success: function(data) {
					alert('Success');
