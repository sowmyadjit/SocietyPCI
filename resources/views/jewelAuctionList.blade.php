...
<?php /*								<td><input class="chkboxs" id="{{$jew->JewelLoanId}}" type="checkbox" /></td>*/?>
								...
								<td><input class="form-control" id="charges_{{$jew->JewelLoanId}}" type="text" /></td>
...
/*		$("#auction_all").click(function(){
		...
		$("[id^='auction_bt_']").click(function() {
				...
				charges = $('#charges_'+jl_alloc_id).val();
				$("#amt").val(amt);
...
//				auc_amt = amt;
				...
				$.ajax({
							url:'/jewelAuctionPay',
							type:'get',
							data:'&jl_alloc_id='+jl_alloc_id+'&cname='+cname+'&ln_no='+ln_no+'&st_date='+st_date+'&end_date='+end_date+'&gross_wt='+gross_wt+'&net_wt='+net_wt+'&ln_amt='+ln_amt+'&rem_amt='+rem_amt+'&auc_amt='+auc_amt+'&rem_int='+rem_int+'&charges='+charges,
							success:function(data)
							{
								alert("success");
								$('#content').html(data);
