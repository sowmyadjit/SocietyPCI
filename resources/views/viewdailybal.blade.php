			...
			<table class="table table-striped table-bordered bootstrap-datatable datatable responsive">
				...
				<tbody>
					...
					@foreach ($trandaily['pigmypayamt_per_adjust'] as $pigmyamt)
						<?php
							$pigmypay_adj_db = $pigmyamt->PgmTotal_Amt;
							$pigmypay_adj_db_total += $pigmypay_adj_db;
						?>
						<tr>
							<td>{{ $pigmyamt->PayAmountReport_PayDate }}</td>
				...
				//		$gt_adj_db += $fd_adj_db_total;
					...
					@foreach ($trandaily['plrepay'] as $plrepay)
						<?php
							$plrepay_cash_cr = $plrepay->PLRepay_PaidAmt;
					...
					@foreach ($trandaily['plrepay_adjust'] as $plrepay)
						<?php
							$plrepay_adj_cr = $plrepay->PLRepay_PaidAmt;
					...
					@foreach ($trandaily['jlrepay'] as $jlrepay)
						@if(strcasecmp($jlrepay->JLRepay_PayMode, 'CASH') == 0)
							<?php
								$jlrepay_cash_cr = $jlrepay->JLRepay_PaidAmt;
						...
						@else
							<?php
								$jlrepay_adj_cr = $jlrepay->JLRepay_PaidAmt;
					...
					<?php
						...
						$b2b_adj_db_total = 0;
					?>
					@foreach ($trandaily['branch_branch_tran'] as $branch_branch_tran)
						@if($branch_branch_tran->Branch_payment_Mode != 'ADJUSTMENT')
						<?php
						...
						@endif
					@endforeach
					@foreach ($trandaily['branch_branch_tran_credit'] as $branch_branch_tran)
						@if($branch_branch_tran->Branch_payment_Mode != 'ADJUSTMENT')
							<?php
	...
	<!---------------salary------------------->
				...
				<tr><td colspan="9"><h5><b><center>Employee Salary<center></b></h5></td></tr>
					<?php
						...
						$emp_sal_adj_db_total = 0;
					?>
					@foreach ($trandaily['emp_sal'] as $row)
						<?php
							$emp_sal_adj_db = $row->gearning;
							$emp_sal_adj_db_total += $emp_sal_adj_db;
						?>
						<tr>
							<td>{{$row->date}}</td>
					...
					@foreach ($trandaily['agent_sal'] as $row)
						<?php
							$emp_sal_adj_db = $row->Agent_Commission_PaidAmount + $row->Tds + $row->securityDeposit ;
							$emp_sal_adj_db_total += $emp_sal_adj_db;
						?>
						<tr>
					...
					<tr>
						...
						<td>-</td>
						<td>-</td>
						<td><?php echo $emp_sal_adj_db_total; ?></td>
						<td>-</td>
						<td>-</td>
					</tr>
					<?php
						$gt_adj_db += $emp_sal_adj_db_total;
					?>
